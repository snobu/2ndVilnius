...
This is a fork of the sample provided at https://github.com/openai/openai-cua-sample-app by OpenAI. This fork can also use Azure OpenAI Computer Use models, in addition to OpenAI.

## Using Azure OpenAI

You can use this project with a model deployed in Azure OpenAI instead of openai.com.
...
1. Set these environment variables in your `.env` file:
   ...
   AZURE_OPENAI_API_VERSION="2024-02-15-preview"  # or your preferred API version
   ```

2. The code will automatically use Azure OpenAI if these variables are set. Otherwise, it will use openai.com.

3. The endpoint for the responses API will be:
   `https://<your-resource-name>.openai.azure.com/openai/deployments/<your-deployment-name>/responses?api-version=<api-version>`

4. The authentication header will use `api-key` instead of `Authorization: Bearer ...`.
...
Or use `uv` for faster package management - https://docs.astral.sh/uv/
